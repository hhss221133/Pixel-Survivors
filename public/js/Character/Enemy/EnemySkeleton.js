const EnemySkeleton = function(ctx, x, y, gameArea, enemyID) {
    ...
    enemy.CreateSpriteSequences(sequences, sequences.idleRight, scale = 1.3, referenceLists.SkeletonOriginal, referenceLists.SkeletonWhite);
    ...
    const GetAttackHitBox = function() {
        ...
        const size = enemy.getDisplaySize();
        const {x, y} = enemy.getXY();
        const top = y - size.height * 0.2 - (enemyScale * 2);
        const bottom = y + size.height * 0.2;
        let left, right;

        if (enemy.getCurSequence().isLeft) {
            left = x - size.width * 0.2 - (enemyScale * 45);
            right = x + size.width * 0.15;
        }
        else {
            left = x - size.width * 0.15;
            right = x + size.width * 0.2 + (enemyScale * 45);
        }
