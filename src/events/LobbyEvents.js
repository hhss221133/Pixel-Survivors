...
function LobbyEvents(socket, io, userTimeouts) {
    ...
    socket.on('lobby char change', (char) => {
        console.log(socket.request.session.username, "sent host char change");
        LobbiesModel.editLobbyChar(socket.request.session.username, socket.request.session.roomID, char, (err, updatedLobby) => {
            if (err) {
                // console.error('Error joining lobby:', err);
                ...
                io.to(socket.request.session.roomID).emit('lobby updated', updatedLobby);
                // io.emit('io updated lobbies json'); //multicast
            }
        });
    });
